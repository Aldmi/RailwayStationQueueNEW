    ...
    public class Сashier : ICasher
    {
        ...
        public TicketItem StartHandling()
        {
            ...
            if (CurrentTicket != null)//TODO:???
            {
                ...
                _loggerCashierInfo.Info($"Команда от кассира: \"StartHandling. (Отправка синхронизации билета)\"  Id= {Id}  NameTicket= {CurrentTicket}");//LOG
            ...
            if (_internalQueueTicket != null && _internalQueueTicket.Any())
            {
                ...
                _loggerCashierInfo.Info($"Команда от кассира: \"StartHandling (внутреняя очередь)\"  Id= {Id}  NameTicket= {newTicket}");//LOG
                return newTicket;
            }
            ...
            if (!_queueTicket.IsEmpty && CurrentTicket == null)
            {
                ...
                if (newTicket != null)
                {
                    ...
                    {
                        _loggerCashierInfo.Info($"Команда от кассира: \"StartHandling (!!!!!!!!!!!Билет из очереди не должен быть в обработке кассиром)!!!!!!!!!!\"  Id= {Id}  NameTicket= {newTicket}");//LOG
                        //return null;//TODO: Прверить лог на возниконверние этого условия
                    }

                    newTicket.Cashbox = Id;
                    _loggerCashierInfo.Info($"Команда от кассира: \"StartHandling (Работаем с внешней очередью)\"  Id= {Id}  NameTicket= {newTicket}");//LOG
                    return newTicket;
                }
            }
            return null;
        ...
        public void SuccessfulStartHandling()
        {
            ...
            if (CurrentTicket != null) 
            {
                ...
                _loggerCashierInfo.Info($"Команда от кассира: \"SuccessfulStartHandling (Ответ на Отправка синхронизации билета)\"  Id= {Id}  NameTicket= {CurrentTicket}");//LOG
                return;
            }
            ...
            if (_internalQueueTicket != null && _internalQueueTicket.Any())
            {
                ...
                CurrentTicket.CountTryHandling++;
                _loggerCashierInfo.Info($"Команда от кассира: \"SuccessfulStartHandling (внутреняя очередь)\"  Id= {Id}  NameTicket= {CurrentTicket}");//LOG
                return;
            }
            ...
            if (!_queueTicket.IsEmpty && CurrentTicket == null)
            {
                ...
                if (newTicket != null)
                {
                    ...
                    CurrentTicket = newTicket;
                    CurrentTicket.CountTryHandling++;
                    _loggerCashierInfo.Info($"Команда от кассира: \"SuccessfulStartHandling (Работаем с внешней очередью)\"  Id= {Id}  NameTicket= {CurrentTicket}");//LOG
                }
            }
        }
        ...
        {
            _loggerCashierInfo.Info($"Команда от кассира: \"SuccessfulHandling (Успешная обработка клиента.)\"  Id= {Id}  NameTicket= {CurrentTicket}");//LOG
            CurrentTicket = null;
            return CurrentTicket;
        }
        ...
        public TicketItem ErrorHandling()
        {
            ...
            _loggerCashierInfo.Info($"Команда от кассира: \"ErrorHandling (Клиент не обрабаотанн.)\"  Id= {Id}    CountTryHandling= {CurrentTicket?.CountTryHandling}  NameTicket= {CurrentTicket}");//LOG
            CurrentTicket = null;
            return CurrentTicket;
        }
        ...
        {
            _loggerCashierInfo.Info($"Команда от кассира: \"AddRedirectedTicket (Добавить перенаправленный билет.)\"  Id= {Id}  NameTicket= {ticket}");//LOG
